...
import { BriefcaseBusiness, FileUser, Home, MessageCircle, PlusCircle } from 'lucide-react'
    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 hidden max-md:block">
      <div className="bg-blur-baseline border-border-subtlest-tertiary grid w-full auto-cols-fr grid-flow-col items-center justify-between border-t bg-background/80 shadow-[0_4px_30px_rgba(0,0,0.1)] backdrop-blur-[2.5rem]">
        <nav className="flex items-center justify-around p-2">
          <Link href="/" prefetch={false} className={cn('flex flex-col items-center gap-1 p-2 text-sm transition-colors', pathname === '/' ? 'text-foreground' : 'text-muted-foreground')}>
          ...
          <Link href="#" prefetch={false} className={cn('flex flex-col items-center gap-1 p-2 text-sm transition-colors', pathname === '/squads' ? 'text-foreground' : 'text-muted-foreground')}>
