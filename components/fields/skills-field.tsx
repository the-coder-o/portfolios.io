                    ...
                    <div className="rounded-xl border bg-card p-2">
                      <div className="flex flex-col gap-1">
                        {filteredSkills.map((skill, index: number) => (
                          <Button type={'button'} key={index} variant="ghost" size="sm" className="flex w-full items-center justify-start gap-2 rounded-xl px-2" onClick={() => handleAddSkill(skill)} disabled={isSkillSelected(skill.name) || selectedSkills.length >= 10}>
                            <span className="flex-1 text-left">{skill.name}</span>
                            <Plus className="h-4 w-4" aria-hidden="true" />
                          </Button>
                        ))}
                        {isCustomSkill && (
                          <Button type={'button'} variant="ghost" size="sm" className="flex w-full items-center justify-start gap-2 rounded-xl px-2" onClick={handleAddCustomSkill} disabled={selectedSkills.length >= 10}>
                    ...
                    <>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.slice(0, 17).map((skill) => (
                          <Button type={'button'} key={skill.name} variant="outline" size="sm" className="flex items-center gap-2 rounded-xl" onClick={() => handleAddSkill(skill)} disabled={isSkillSelected(skill.name) || selectedSkills.length >= 10}>
                            {skill.name}
                            <Plus className="h-4 w-4" aria-hidden="true" />
                          </Button>
                        ))}
